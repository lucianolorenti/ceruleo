...
In the context of remaining useful life regression and other predictive maintenance (PdM) tasks, there exists a wide range of repositories containing published and unpublished models, as well as multiple libraries dedicated to feature extraction in time series data. However, it is challenging to find libraries that effectively combine both aspects, focusing on predictive maintenance while also incorporating tools for model comparison.

To the best of my knowledge, the prog_models and prog_als libraries developed by NASA [@2022_nasa_prog_models] come closest to fulfilling these requirements. Although these libraries have a broader scope, including simulation capabilities, they lack extensive mechanisms for feature extraction from time series data. They do provide LSTM models and Dynamic Mode Decomposition, wrapped in their own library-specific classes, which can be utilized in conjunction with the prog_server library. Conversely, CeRULEo primarily emphasizes a data-driven approach using industrial data, particularly in cases where a clear simulation model may be unavailable or expensive to develop. Notably, the CeRULEo library adheres to the guiding principle of avoiding additional classes that wrap the models themselves, enabling seamless deployment in any production environment. The objective is to ensure maximum model library-agnosticism.
