...
def compute_rul_line(rul: float, n: int, tt: np.array):
    z = np.zeros(n)
    z[0] = rul
...
def preventive_ruls(train_dataset, test_dataset, window, transform=lambda x: x):
    ruls = [transform(life[test_dataset.rul_column].iloc[0])
...
def cv_predictions(model: TrainableModel,
                   dataset: AbstractLivesDataset,
                   cv=KFold(n_splits=5),
                   fit_params={},
                   progress_bar=False) -> Tuple[List[List], List[List]]:
    """
...
def cv_error_histogram(y_true: List[List], y_pred: List[List], nbins: int = 5) -> Tuple[np.array, np.array]:
    """
    ...
    * error_histogram: An array of len(y_true) containing the mse of the points
    ...
    for y_pred, y_true in zip(y_pred, y_true):
        for j in range(len(bin_edges)-1):
            indices = np.where((y_true >= bin_edges[j]) & (
                y_true <= bin_edges[j+1]))[0]
            if len(indices) > 0:
                error_histogram[j].append(
                    np.sqrt(mse(y_true[indices], y_pred[indices])))
