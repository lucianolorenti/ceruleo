...
import pandas as pd
from rul_pm.transformation.features.extraction_numba import (
    compute,
    roll_matrix,
    stats_order,
)
...
class ExpandingStatistics(TransformerStep):
    ...
    def _kurtosis(
        self, s: Expanding, s_abs: Expanding, s_abs_sqrt: Expanding, s_sq: Expanding
    ):
        return s.kurt(skipna=True)
    ...
    def _impulse(
        ...
        return self._peak(s, s_abs, s_abs_sqrt, s_sq) / s_abs.mean()

    def _clearance(
        self, s: pd.Series, s_abs: Expanding, s_abs_sqrt: Expanding, s_sq: Expanding
    ):
        return self._peak(s, s_abs, s_abs_sqrt, s_sq) / s_abs_sqrt.mean().pow(2)
    ...
    def _mean(self, s: pd.Series, s_abs: Expanding, s_abs_sqrt: Expanding, s_sq: Expanding):
        return s.mean()

    def _shape(self, s: pd.Series, s_abs: Expanding, s_abs_sqrt: Expanding, s_sq: Expanding):
        return self._rms(s, s_abs, s_abs_sqrt, s_sq) / s_abs.mean(skipna=True)

    def _crest(self, s: pd.Series, s_abs: Expanding, s_abs_sqrt: Expanding, s_sq: Expanding):
        return self._peak(s, s_abs, s_abs_sqrt, s_sq) / self._rms(s, s_abs, s_abs_sqrt, s_sq)
        ...
        for c in X.columns:
            for stats in self.to_compute:
                X_new[f"{c}_{stats}"] = getattr(self, f"_{stats}")(
                    expanding[c], s_abs[c], s_abs_sqrt[c], s_sq[c]
                )
