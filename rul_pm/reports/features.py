...
def null_proportion_per_life(dataset):
    ...
    for i, life in enumerate(dataset):
        ...
        data.append((i, life.shape[1], number_of_completely_null,   number_of_half_null,
                     number_of_25p_null, mean_null_proportion))
    df = pd.DataFrame(data,
                      columns=['Life',
                               ...
                               'Mean null propotion'])
    df.sort_values(by='Number of completely null features',
    ...
    for life in dataset:
        ...
        for column in comon_features:
            ...
            if not isinstance(d[column], float):
                continue
            std_list = std_per_life.setdefault(
                column, [])
