...
def transformation_pipeline(outlier=IQROutlierRemover(), imputer=NaNRemovalImputer(),
    ...
    return Pipeline(steps=[
        ...
        (RESAMPLER_STEP_NAME, resampler if resampler is not None else 'passthrough'),
...
class Transformer:
    ...
    def transformY(self, df):
        return np.squeeze(self.transformerY.transform((df[[self.time_feature, self.target_column]])))
