...
from ceruleo.dataset.analysis.correlation import (
    CorrelationAnalysis,
    correlation_analysis,
)
from ceruleo.transformation.features.selection import ByNameFeatureSelector
from ceruleo.transformation.functional.transformers import Transformer
from ceruleo.dataset.analysis.sample_rate import (
    SampleRateAnalysis,
    sample_rate,
    sample_rate_summary,
)
...
class MockDataset(AbstractPDMDataset):
    def __init__(self, nlives: int):
        ...
        self.lives = [
            pl.DataFrame(
                {
                    "timestamp": np.linspace(0, 100, 50),
            ...
            pl.DataFrame(
                {
                    "timestamp": pl.date_range(
                        start=datetime(2022, 7, 5, 1, 1, 1),
                        end=datetime(2022, 7, 5, 1, 1, 50),
                        interval="1s",
                        eager=True
                    ),
                    "feature1": np.linspace(0, 100, 50),
...
class TestAnalysis:
    def test_correlation(self):
        ...
        assert isinstance(
            correlation_analysis(transformed_dataset), CorrelationAnalysis
        )
